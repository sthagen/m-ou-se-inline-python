...
use pyo3::{Bound, IntoPyObject, PyErr, PyResult, PyTypeInfo, Python, exceptions::PyBaseException, prelude::*, types::PyTraceback};
use std::{
...
pub(crate) fn python_from_macro(input: TokenStream, variables: Option<&mut BTreeMap<String, Ident>>) -> Result<String, TokenStream> {
	struct Location {
	...
	fn add_whitespace(python: &mut String, loc: &mut Location, span: Span) -> Result<(), TokenStream> {
		let line = span.line();
...
pub(crate) fn compile_python(py: Python<'_>, python: &CStr, filename: &CStr, tokens: TokenStream) -> Result<Py<PyAny>, TokenStream> {
	unsafe {
		pyo3::PyObject::from_owned_ptr_or_err(
			py,
			pyo3::ffi::Py_CompileString(python.as_ptr(), filename.as_ptr(), pyo3::ffi::Py_file_input),
		)
	}
...
pub(crate) fn python_error_to_compile_error(py: Python, error: PyErr, tokens: TokenStream) -> TokenStream {
